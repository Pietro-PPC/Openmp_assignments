...
int main(int argc,char *argv[]) {
	...
	MPI_Init(&argc,&argv);
 	...
 	gettimeofday(&ini, NULL); // começa a contar tempo sequencial da alocação
	...
	MPI_Barrier(MPI_COMM_WORLD);
 	...
 	gettimeofday(&fim, NULL); //  termina contagem de tempo de alocação e soma no tempo sequencial
	seq_time += fim.tv_sec*1e6 + fim.tv_usec - (ini.tv_sec*1e6 + ini.tv_usec);
	...
	int reps = 0;
	printf("%d\n", rank);
	gettimeofday(&ini, NULL); // começa contagem de tempo sequencial dentro do for
	for(i=0;i<n;i++){
	...
	gettimeofday(&fim, NULL); // termina contagem de tempo sequencial dentro do for
	float avg_time = fim.tv_sec*1e6 + fim.tv_usec - (ini.tv_sec*1e6 + ini.tv_usec); // calcula tempo total das repetições em microssegundos
	avg_time /= (float)reps; // calcula tempo médio dividindo tempo total por numero de repetições
	// seq_time += avg_time * n; // o for externo não tem como paralelizar, então multiplica por n
	
	printf("Tempo sequencial total: %.2fus + %.2fus * n\n", seq_time, avg_time);
